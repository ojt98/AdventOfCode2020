...
class Boarding_check:  # a class containing methods that implement the logic for finding the upper and lower limits
    # for letters B anf F
    ...
    def upper_half(self, type):  # For letter B in a boarding pass we need to get the upper half of the range
        ...
        if self.lower_bound == self.upper_bound:
            type.append(
                self.lower_bound)  # if upper_bound and lower_bound converge on a B, we take this value as the row
            # number
    ...
    def lower_half(self, type):  # For letter F in a boarding pass we need to get the lower half of the range
        ...
        if self.lower_bound == self.upper_bound:
            type.append(
                self.lower_bound)  # if upper_bound and lower_bound converge on an F, we take this value as the row
            # number
...
class Columns(Boarding_check):  # A class that contains a method for constructing a new list with the column numbers
    # as elements
